...
The Fenwick tree, also known as the Binary Indexed Tree, is a data structure that can perform range operations such as finding the total, minimum, or maximum value in  $O(log N)$  time. 

It can also perform point updates in  $O(log N)$  time. However, it does not support range updates; in such cases, a segment tree should be used instead. 

Despite being called a tree, the Fenwick tree is actually stored as an array of elements.

## Fenwick Tree Function
...
## Update Element in Fenwick Tree
...
Delta at $0{th}$ index is $3 − 1 = 2$ , so we start by adding $2$ starting from $1$ st index in fenwick tree.
