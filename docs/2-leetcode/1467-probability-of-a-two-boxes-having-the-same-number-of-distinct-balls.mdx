...
Each of the $\text{N}$ indices in the input array has $2$ boxes to choose from, so total time complexity will be:
...
Since there are $\text{N}$ indices to assign to each box, the stack size for the backtracking will be $\text{N}$, so space complexity will be:
