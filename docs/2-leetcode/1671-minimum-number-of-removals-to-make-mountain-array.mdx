...
Each of the $\text{N}$ indices in the input array is visited only once, so time complexity is:
...
Solution uses $\text{O(N)}$ space for storing longest increasing sequence from right and left, so space complexity is:
