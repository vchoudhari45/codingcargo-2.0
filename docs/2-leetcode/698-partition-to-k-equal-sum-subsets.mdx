...
To solve this problem using backtracking, think of it like the $\text{N}$ Queen problem, where you try to assign each element of an array to one of the $\text{K}$ subsets. This approch will require time complexity of $\text{K}^\text{N}$. 

For smaller values of $\text{K}$, backtracking works best. However, when $\text{K}$ grows larger, time complexity of $\text{K}^\text{\text{N}}$ will result in a time limit exceeded exception.
...
For our sample input $\text{nums} = \begin{bmatrix}1,2,3\end{bmatrix}$, when our algorithm reaches a node with the subset $[1, 2]$, it finds that subset sum is $3$, which matches the required equal sum. 
...
Once we have generated total $\text{K}$ subsets and verified that each subset sum equals $3$, we return true, else return false.
