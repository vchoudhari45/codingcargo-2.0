...
The problem is asking us to cut the ribbons into one or more parts, such that each of the $K$ parts will have an equal maximum length. We are also allowed to discard any extra parts of the ribbons if required.
...
Using binary search, we first assume a candidate value($\text{mid}$), and attempt to cut the ribbon at $\text{i}^{th}$ place so that at least one of the part has length equals to the candidate value. 
...
This is because, in the next iteration of binary search, we have candidate value of $7$. If we can't have $3$ ribbons of length $7$, then $5$ should be answer.
:::
...
Let's say there are $N$ elements in an array, and $S$ is the length of the longest ribbon.
