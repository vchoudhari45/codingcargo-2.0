...
Using binary search, we first assume a candidate value($\text{mid}$), and attempt to split the array into $K$ subarrays with sums less than or equal to candidate value.
