...
This problem can be solved using the Binary Search technique to split the array into $K$ subarrays. More such questions can be found [here](/binary-search/split-array-into-k-subarrays).
...
This is because, in the next iteration of binary search, we might find a candidate value (let's call it $X$) that is less than $21$, and if $X$ does not allow splitting the array into $K$ subarrays with all subarray sums less than or equal to $X$, then $21$ should be our answer.
:::
...
class Solution {
    ...
    // Helper function to calculate number of partitions given maximum subarray sum
