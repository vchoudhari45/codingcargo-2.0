...
The problem is asking us to divide the jobs among $K$ workers in such a way the maximum working time of any worker is minimized.
...
:::info[Assumption]
Assume job which takes $3$ units of time is called $P$, job which takes $2$ units of time is called $Q$ and job which takes $4$ units of time is called $R$.
:::

We start by assigning $P$ jobs among $2$ workers, which gives us $2$ possibilities. Next we assign $Q$ and $R$ jobs in the next iterations, which gives us total of $8$ possibilities.

Below is a graphical representation of the distribution of $3$ jobs $P, Q, R$ among $2$ workers: 
    ...
    private void backtrack(int[] jobs, int index, int[] workers) {
        ...
        if (index == jobs.length) {
            ...
            for (int i = 0; i < workers.length; i++) {
                ...
                /**
                 * If the current assignment exceeds the previously found min, 
                 * then discard
                 */
