...
Using binary search, we first assume a candidate value($\text{mid}$), and attempt to split the array(Chocolate in this case) into $K$ subarrays with sums greater than or equal to candidate value.
