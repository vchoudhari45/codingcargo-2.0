...
The problem requires us to calculate the probability that two boxes will have the same number of distinct balls after shuffling $2N$ balls into two boxes, such that each box containing $N$ balls.
...
```java
class Solution {
    // Array to store pre-calculated factorials
    ...
    private double backtrack(int[] balls, int index, int box1Color, int box2Color,
      int box1Count, int box2Count) {
        ...
        if (index == balls.length) {
            /**
             * Check box1Count == box2Count to determine if the distribution is valid, 
             * and box1Color == box2Color to verify if the number of distinct colors 
            ...
            for (int i = 0; i <= balls[index]; i++) {
                /**
                 * This combinations accounts for number of ways i balls can choosen 
                 * from balls[index] balls. 
                 */
...
:::info[]
Here, we ignore time complexity required to distribute elements at each of the $N$ indices into $2$ boxes, because the input constraint $1 \leq \text{balls}[\text{i}] \leq 6$ ensures that this number will be very small.
:::
