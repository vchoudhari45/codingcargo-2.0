...
class Solution {
    ...
    public int minimumXORSum(int[] nums1, int[] nums2) {
        memo = new HashMap<>();
        return backtrack(nums1, nums2, 0, 0);
    }

    // Backtracking method to find the minimum XOR sum
    private int backtrack(int[] nums1, int[] nums2, int index, int mask) {
        ...
        if (index == nums2.length) {
            ...
            for (int i = 0; i < nums2.length; i++) {
                ...
                int internalAns = (nums1[index] ^ nums2[i]) + backtrack(
                        nums1,
                        nums2,
                        index + 1,
                        mask | (1 << i)
